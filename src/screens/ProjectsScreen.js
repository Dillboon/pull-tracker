...
import {
  ...
  FlatList, StyleSheet, Alert, Modal,
} from 'react-native';
import { COLORS, DEFAULT_IDFS, DEFAULT_DEVICE_TYPES } from '../theme';
...
export default function ProjectsScreen({ projects, setProjects, onOpenProject }) {
  ...
  const archiveProject = (id) => {
    Alert.alert('Archive Project', 'Archive this project? You can restore it later.', [
  ...
  const deleteProject = (id) => {
    ...
    Alert.alert('⚠️ Delete Project',
      `Permanently delete "${proj?.name}" with ${proj?.drops.length} drops and ${proj?.devices?.length ?? 0} devices?`,
      [
        { text: 'Cancel', style: 'cancel' },
...
const s = StyleSheet.create({
  header: {
    ...
    borderBottomColor: COLORS.border, padding: 14,
  ...
  newBtn: {
    ...
    borderColor: 'rgba(59,130,246,0.4)', borderRadius: 8,
    paddingHorizontal: 14, paddingVertical: 8,
  },
  newBtnText:   { color: COLORS.blue, fontWeight: '800', fontSize: 13 },
  sectionLabel: { fontSize: 10, fontWeight: '800', letterSpacing: 1, color: COLORS.textMuted, marginBottom: 12 },
  ...
  archivedToggleText: { fontSize: 11, fontWeight: '800', color: COLORS.textMuted, letterSpacing: 0.8 },
  ...
  modalInput: { backgroundColor: 'rgba(255,255,255,0.05)', borderWidth: 1, borderColor: COLORS.border, borderRadius: 8, padding: 12, color: COLORS.text, fontSize: 14 },
